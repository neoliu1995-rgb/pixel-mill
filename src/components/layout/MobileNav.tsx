  ...
  const navItems = [
    {
      ...
      label: t.nav.home,
      icon: (active: boolean) => (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={`w-6 h-6 ${active ? "text-purple-600" : "text-gray-500"}`}>
      ...
      <div className="flex items-center justify-around h-16">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {item.icon(active)}
              <span className={`text-xs ${active ? "font-semibold text-purple-600" : "text-gray-500"}`}>
                {item.label}
              </span>
            </Link>
          );
