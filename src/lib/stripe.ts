...
export const PRICE_TO_PLAN: Record<string, { plan: string; billingPeriod: string }> = {
  ...
  [PRICES.proYearly]: { plan: "pro", billingPeriod: "yearly" },
  [PRICES.businessMonthly]: { plan: "business", billingPeriod: "monthly" },
  [PRICES.businessYearly]: { plan: "business", billingPeriod: "yearly" },
};

export function mapPriceToPlan(priceId: string): { plan: string; billingPeriod: string } | null {
...
export const CNY_PLANS = {
  ...
  pro: {
    ...
    description: "最受欢迎的选择",
    monthlyPrice: 49.9,
    originalMonthlyPrice: 69.9,
    yearlyPrice: 399,
    originalYearlyPrice: 699,
    promotional: true,
    features: [
      "300张/月",
      "FLUX.2 Pro + 通义万相",
...
export const CURRENCY_PLANS: Record<"usd" | "cny", typeof PLANS> = {
  usd: PLANS,
  cny: CNY_PLANS,
};

export function getPlansForCurrency(currency: "usd" | "cny") {
  return CURRENCY_PLANS[currency];
}